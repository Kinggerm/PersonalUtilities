        ...
        if annotation_lines[i][0] in ['gene', 'tRNA', 'rRNA', 'exon', 'intron', 'CDS'] and 'gene=' in ''.join(annotation_lines[i][2:]):
            ...
            for l in range(0, len(region_parts)):
                ...
                for j in range(2, len(annotation_lines[i])):
                    this_dict[annotation_lines[i][j].split('=')[0]] = '='.join(annotation_lines[i][j].split('=')[1:]).strip('"')
...
def write_vista_line(regions, which, out, start_with_direction=True):
...
def vista_formate_with_gb(this_gb_dir):
    ...
    region_list = read_annotation_of_gb(annotation_lines, seq_len, False)['by_region']
    # ---------------------------------------------------------------------------
    ...
    while region_list:
        if len(region_list) > 1:
            if not region_list[0][2]['gene'] == region_list[1][2]['gene']:
                ...
                write_vista_line(region_list, 1, out_put)
                write_vista_line(region_list, 0, out_put, False)
                i = 0
                while region_list and region_list[i][1] <= this_gene[1]:
                    if region_list[i][2]['gene'] == this_gene[2]['gene']:
                        write_vista_line(region_list, i, out_put, False)
                    else:
                        i += 1
        else:
            write_vista_line(region_list, 0, out_put)
