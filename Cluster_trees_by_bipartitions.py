...
def delete_suffix(tab_matrix, is_titled=False, column=0):
    ...
    while i > 0:
        ...
        while j < len(del_suffix_matrix):
            if del_suffix_matrix[is_titled][column][
               (len(del_suffix_matrix[is_titled][column]) - i):len(del_suffix_matrix[is_titled][column])] == \
                    del_suffix_matrix[j][column][
                    (len(del_suffix_matrix[j][column]) - i):len(del_suffix_matrix[j][column])]:
...
def transform_to_bipartition(tree_line):
    ...
    for i in range(0, len(parentheses_pairs[0])):
        ...
        if len(temp_set1) >= 2 and len(temp_set2) >= 2:
            ...
            temp_num = 0
            try:
                is_int = float(tree_line[parentheses_pairs[1][i] + 1]) == int(
                    float(tree_line[parentheses_pairs[1][i] + 1]))
            except:
...
def get_options():
    ...
    parser = OptionParser(usage="Usage: python Cluster_trees_by_bipartitions.py "
                                "-b bipartition_file -o out_put_dir list_of_newick_trees\n"
                                "by jinjianjun@mail.kib.ac.cn in python2")
    parser.add_option("-b", dest="bipartition_file",
                      help="Each line in this file assigns a kind of bipartition:\t"
                           "\"Tag_1\\ttaxon1,taxon2\" would returns a cluster of trees that have "
                           "bipartition \"taxon1,taxon2|other_taxa\"."
                           "\"Tag_2\\ttaxon1\\ttaxon2\" would returns a cluster of trees that DO NOT have "
                           "bipartition \"taxon1,taxon2|other_taxa\".")
    parser.add_option("-s", dest="support", default=70, type=float,
                      help="The support bootstrap threshold under which the bipartition would not be considered. "
                           "Default: 70.")
...
def main():
    ...
    bootstrap_threshold = options.support
    if not os.path.exists(options.output):
        os.mkdir(options.output)
    statistics_for_trees = [['Tree']]
    criteria_tab = read_tab(options.bipartition_file)
    criteria = []
    separate_taxa_in_criteria = []
    for i in range(0, len(criteria_tab)):
        if len(criteria_tab[i]) == 2:
            criteria.append(
                [criteria_tab[i][0], set(criteria_tab[i][1].replace('"', '').replace(', ', ',').split(','))])
            ...
            criteria.append([criteria_tab[i][0], set(criteria_tab[i][1].replace('"', '').replace(', ', ',').split(',')),
                             set(criteria_tab[i][2].replace('"', '').replace(', ', ',').split(','))])
        os.mkdir(os.path.join(options.output,
                              'ClusterTrees_with_' + criteria[i][0] + '_Threshold_' + str(bootstrap_threshold)))
    ...
    for i in range(0, len(tree_files)):
        read_tree = open(tree_files[i], 'rU')
            ...
            if not separate_taxa_in_criteria[j]:
                ...
                if (criteria[j][1] in bipartition_matrix[0]):
                    if bipartition_matrix[1][bipartition_matrix[0].index(criteria[j][1])] >= bootstrap_threshold:
                        os.system('cp ' + tree_files[i] + ' ' + os.path.join(options.output,
                                                                             'ClusterTrees_with_' + criteria[j][
                                                                                 0] + '_Threshold_' + str(
                                                                                 bootstrap_threshold)))
                    statistics_for_trees[i + 1].append(
                        bipartition_matrix[1][bipartition_matrix[0].index(criteria[j][1])])
                else:
                    statistics_for_trees[i + 1].append('-')
            else:
                # check
                if not print_warn_for_this_criteria[j] and not set(
                                list(criteria[j][1]) + list(criteria[j][2])).issubset(taxa_names):
                    print_warn_for_this_criteria[j] = True
                    print 'Error in Criteria tab file: Line ' + str(j + 1) + ': ' + criteria_tab[j][1] + '\t' + \
                          criteria_tab[j][2]
                #
                if (criteria[j][1] in bipartition_matrix[0]) and (criteria[j][2] in bipartition_matrix[0]) and (
                    set(list(criteria[j][1]) + list(criteria[j][2])) not in bipartition_matrix[0]):
                    conflit_node_bs = []
                    for k in range(0, len(bipartition_matrix[0])):
                        if (len(bipartition_matrix[0][k]) < len(taxa_names) - 1) and (
                            len(bipartition_matrix[0][k]) > 1) and abs(
                                        criteria[j][1].issubset(bipartition_matrix[0][k]) - criteria[j][2].issubset(
                                        bipartition_matrix[0][k])):
                            conflit_node_bs.append(bipartition_matrix[1][k])
                    if bipartition_matrix[1][bipartition_matrix[0].index(criteria[j][1])] >= bootstrap_threshold and \
                                    bipartition_matrix[1][
                                        bipartition_matrix[0].index(criteria[j][2])] >= bootstrap_threshold and max(
                            conflit_node_bs) >= bootstrap_threshold:
                        os.system('cp ' + tree_files[i] + ' ' + os.path.join(options.output,
                                                                             'ClusterTrees_with_' + criteria[j][
                                                                                 0] + '_Threshold_' + str(
                                                                                 bootstrap_threshold)))
                    statistics_for_trees[i + 1].append(
                        min(max(conflit_node_bs), bipartition_matrix[1][bipartition_matrix[0].index(criteria[j][1])],
                            bipartition_matrix[1][bipartition_matrix[0].index(criteria[j][2])]))
                else:
                    statistics_for_trees[i + 1].append('-')
    statistics_for_trees = delete_suffix(delete_prefix(statistics_for_trees, True, 0), True, 0)
    write_tab(statistics_for_trees, os.path.join(options.output, 'Statistics_for_trees'))
